...
Debug defaults to mock services and verbose logs. Prod disables dangerous debug features, minimizes logs, and sanitizes sensitive fields such as password, token, authorization, and cookie.
...
Any valid email and password with at least six characters can log in or register in debug/mock mode. Tokens are stored through the secure storage abstraction. On Web, secure browser storage still carries XSS risk; production apps should pair it with a hardened CSP, short-lived access tokens, refresh-token rotation, and server-side session invalidation.

## API Integration

Remote service classes are included for auth and version checks. Replace base URLs and endpoints in `EnvConfig` and implement vendor-specific error mapping in the repository layer. UI must not call dio directly.
...
`.github/workflows/flutter_ci.yml` runs pub get, formatting check, analyze, tests, and a web build. Add signing credentials through GitHub secrets only; do not commit certificates, provisioning profiles, keystores, or API keys.
